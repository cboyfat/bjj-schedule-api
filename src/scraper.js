...
function determineClassType(className) {
  ...
  if (name.includes('no-gi')) return 'no-gi';
  if (name.includes('open mat')) return 'open-mat';
  if (name.includes('wrestling')) return 'wrestling';
  if (name.includes('fundamental') || name.includes('beginner')) return 'fundamentals';
  return 'gi';
}

function determineDifficulty(className) {
  const name = className.toLowerCase();
  if (name.includes('advanced') || name.includes('competition')) return 'advanced';
  if (name.includes('intermediate')) return 'intermediate';
  if (name.includes('beginner') || name.includes('fundamental')) return 'beginner';
  if (name.includes('all levels')) return 'all-levels';
  return null;
}
...
export async function findGymsNearLocation(lat, lng, radiusMiles) {
  ...
  const nearbyGyms = knownGyms
    .map(gym => ({ ...gym, distance: calculateDistance(lat, lng, gym.lat, gym.lng) }))
...
export async function getGymData(gym) {
  ...
  return { ...gym, schedule, lastScraped: new Date().toISOString(), scraped: true };
}
